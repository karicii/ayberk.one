    ...
    case '/api/login':
        if ($request_method === 'POST') {
            ...
            if ($user && password_verify($data['password'] ?? '', $user['password'])) {
                ...
                $payload = [
                    "iss" => $issuer_claim, "aud" => $issuer_claim, "iat" => $issuedat_claim, "exp" => $expire_claim,
        ...
        if (preg_match('/^\/api\/posts\/([a-zA-Z0-9-]+)$/', $request_uri, $matches)) {
            ...
            switch ($request_method) {
                ...
                case 'PUT':
                    ...
                    if ($postRepo->update($slug, $data)) {
                        http_response_code(200);
                        echo json_encode(['message' => 'Yazı başarıyla güncellendi.']);
                    } else {
                        http_response_code(500);
                        echo json_encode(['message' => 'Yazı güncellenirken bir hata oluştu.']);
                    }
                    break;
                
                case 'DELETE':
                    handleAuth();
                    if ($postRepo->delete($slug)) {
                        http_response_code(200);
                        echo json_encode(['message' => 'Yazı başarıyla silindi.']);
                    } else {
                        http_response_code(500);
                        echo json_encode(['message' => 'Yazı silinirken bir hata oluştu.']);
                    }
                    break;

                default:
                    http_response_code(405);
                    echo json_encode(['message' => 'Bu endpoint için izin verilen metotlar: GET, PUT, DELETE']);
                    break;
            }
        } else {
            http_response_code(404);
